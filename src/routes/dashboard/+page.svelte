<script lang="ts">
  ...
  const tenantDocumentFields = ['creditData', 'appData', 'damageData'] as const satisfies readonly TenantDocumentField[];
  ...
  let maintenanceIssues = $state<Array<{ id: string; created?: string; problem?: string; status?: string; image?: string | string[] }>>([]);
  let maintenanceIssuesLoading = $state(false);
  ...
  let selectedDocumentUrl = $state<string | null>(null);
  ...
  async function loadMaintenanceIssuesForCurrentRenter() {
    const viewToken = ++dashboardLoadToken;
    ...
    const tenantProfile = await renterService.getCurrent();
    if (viewToken !== dashboardLoadToken || !tenantProfile?.id) {
      maintenanceIssues = [];
      return;
    }

    maintenanceIssuesLoading = true;

    try {
      const records = await pocketbase.client.collection('maintenance').getFullList({
        sort: '-created',
        filter: `tenant = "${tenantProfile.id}"`
      });
      if (viewToken !== dashboardLoadToken) return;
      maintenanceIssues = records as Array<{ id: string; created?: string; problem?: string; status?: string; image?: string | string[] }>;
    } catch (error) {
      if (error instanceof Error && (error.name === 'AbortError' || error.message?.toLowerCase().includes('aborted'))) {
        return;
      }
      console.error('[dashboard page] failed to load maintenance issues:', error);
      maintenanceIssues = [];
    } finally {
      if (viewToken === dashboardLoadToken) {
        maintenanceIssuesLoading = false;
      }
    }
  }
    ...
    const loadDashboardData = async () => {
      ...
      try {
        ...
        if (userRole !== 'admin') {
          ...
          if (requestToken !== dashboardLoadToken || requestId !== activeRentalRequestId) return;
        ...
        if (userRole === 'admin') {
          try {
            ...
            console.error('[dashboard page] admin tenant query failed:', error);
            tenants = [];
          }
        } else {
          renterProfile = await renterService.getCurrent();
          if (requestToken !== dashboardLoadToken || requestId !== activeRentalRequestId) return;
          await loadMaintenanceIssuesForCurrentRenter();
        }
      } catch (error) {
        const isAutoCancel = error instanceof Error && (error.name === 'AbortError' || error.message?.toLowerCase().includes('aborted'));
  ...
  function openDocumentModal(field: TenantDocumentField, fileName: string) {
    if (!renterProfile?.id || !fileName) return;
    const tenantRecord = {
      id: renterProfile.id,
      user: renterProfile.user
    } as Record<string, any>;
    selectedDocumentUrl = pocketbase.client.files.getURL(tenantRecord, fileName);
...
{#if selectedReceiptUrl}
  <div class="receipt-modal-backdrop" onclick={closeReceiptModal} role="presentation">
    <div class="receipt-modal" tabindex="0" onclick={(event) => event.stopPropagation()} onkeydown={(event) => {
      if (event.key === 'Escape') closeReceiptModal();
    }} role="dialog" aria-modal="true" aria-label={selectedReceiptName}>
...
{#if selectedDocumentUrl}
  <div class="receipt-modal-backdrop" onclick={closeDocumentModal} role="presentation">
    <div class="receipt-modal" tabindex="0" onclick={(event) => event.stopPropagation()} onkeydown={(event) => {
      if (event.key === 'Escape') closeDocumentModal();
    }} role="dialog" aria-modal="true" aria-label={selectedDocumentName}>
      <div class="receipt-modal-header">
        <h3>{selectedDocumentName}</h3>
        <button class="icon-button" type="button" aria-label="Close document" onclick={closeDocumentModal}>×</button>
      </div>

      {#if isImageReceipt(selectedDocumentUrl)}
        <img class="receipt-preview" src={selectedDocumentUrl} alt={selectedDocumentName} />
      {:else}
        <div class="receipt-preview-placeholder">
          <FileText size={36} />
          <p>{selectedDocumentName}</p>
          <a class="button" href={selectedDocumentUrl} target="_blank" rel="noreferrer noopener">Open file</a>
        </div>
      {/if}
    </div>
  </div>
...
{#if userRole !== 'admin' && tenantStatusModal === null && renterProfile && renterProfile.id}
  <section class="panel" style="margin-top: 24px;">
    <p class="page-kicker">Documents</p>
    <h2>Tenant records</h2>
    <div class="document-grid">
      {#each tenantDocumentFields as field}
        <div class="document-card">
          <div class="document-card-header">
            <strong>{field === 'creditData' ? 'Credit data' : field === 'appData' ? 'Application data' : 'Damage data'}</strong>
            <span>{renterProfile[field]?.length ?? 0} file(s)</span>
          </div>

          {#if renterProfile[field]?.length}
            <ul class="document-file-list">
              {#each renterProfile[field] as fileName}
                <li>
                  <button class="file-link-button" type="button" onclick={() => openDocumentModal(field, fileName)}>
          ...
          {:else}
            <p class="muted small-muted">No files uploaded yet.</p>
          {/if}
        </div>
      {/each}
    </div>
  ...
  <section class="panel" style="margin-top: 24px;">
    ...
    {#if maintenanceIssuesLoading}
      <p class="muted">Loading maintenance issues...</p>
    {:else if maintenanceIssues.length}
      <ul class="rental-list">
        {#each maintenanceIssues as issue}
          <li class="rental-row maintenance-row">
            <div class="rental-row-copy">
              <strong>{plainText(issue.problem) || 'Maintenance request'}</strong>
              <span>{issue.created ? new Date(issue.created).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Recent request'}</span>
            </div>
            <span class="status-tag {issue.status ?? 'reported'}">{maintenanceStatusLabel(issue.status)}</span>
          </li>
        {/each}
      </ul>
    {:else}
      <p class="muted">You do not have any open maintenance requests right now.</p>
    {/if}
  </section>
{/if}
