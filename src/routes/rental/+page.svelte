...
<div class="rental-grid"><section class="panel"><h2 class="panel-title">Property information</h2>{#if editing}<div class="field"><label>ID<input value={rental?.id ?? ''} disabled /></label></div><label class="field">Address<input bind:value={address} /></label><label class="field">Monthly rent<input bind:value={rent} type="number" /></label><div class="field"><span>Photos</span><div class="chip-list">{#if rental?.photos?.length}{#each rental.photos as photo}<span class="chip">{photo}</span>{/each}{:else}<span class="muted">---</span>{/if}</div></div><div class="field"><span>Bills</span><div class="chip-list">{#if rental?.bills?.length}{#each rental.bills as bill}<span class="chip">{bill}</span>{/each}{:else}<span class="muted">---</span>{/if}</div></div><div class="field"><label for="rental-photos">Photo upload</label><input id="rental-photos" name="rental-photos" type="file" accept="image/*" multiple onchange={handlePhotoSelection} /></div>{#if pendingPhotos.length}<div class="field"><span>{pendingPhotos.length === 1 ? 'Selected photo' : 'Selected photos'}</span><div class="chip-list">{#each pendingPhotos as file}<span class="chip">{file.name}</span>{/each}{#if photoSizeLabel}<span class="chip size-chip">{photoSizeLabel}</span>{/if}</div><button class="button" type="button" onclick={uploadPhotos} disabled={uploadingPhotos}>{uploadingPhotos ? 'Uploading...' : `Upload ${pendingPhotos.length} photo${pendingPhotos.length === 1 ? '' : 's'}`}</button></div>{/if}{#if photoUploadError}<p class="error-text">{photoUploadError}</p>{#if photoUploadDebug}<pre class="error-debug">{photoUploadDebug}</pre>{/if}{/if}<button class="button" onclick={save}>Save changes</button>{:else}<dl class="detail-list"><div><dt>Address</dt><dd>{plainText(rental?.address) || 'No rental set up'}</dd></div><div><dt>Monthly rent</dt><dd>${(rental?.rent ?? 0).toLocaleString()}</dd></div><div><dt>Renter</dt><dd>{getAssignedRenterLabel()}<span class="muted">{getAssignedRenterEmail() ? ` · ${getAssignedRenterEmail()}` : ''}</span></dd></div></dl>{/if}{#if saved}<p class="success">Rental details saved.</p>{/if}</section><section class="panel"><h2 class="panel-title">Property photos</h2>{#if rental?.photos?.length}<div class="photo-grid">{#each rental.photos as photo}<img class="property-photo" src={pocketbase.client.files.getURL(rental, photo)} alt="Rental property" />{/each}</div>{:else}<div class="photo-placeholder"><Image size={27} /><span>No photos added yet</span></div>{/if}{#if isAdmin}<div class="upload-block"><label class="upload-label" for="property-photo-upload">Upload photos</label><input id="property-photo-upload" type="file" accept="image/*" multiple onchange={handlePhotoSelection} />{#if pendingPhotos.length}<div class="field"><span>Selected photos</span><div class="chip-list">{#each pendingPhotos as file}<span class="chip">{file.name}</span>{/each}</div><button class="button" type="button" onclick={uploadPhotos} disabled={uploadingPhotos}>{uploadingPhotos ? 'Uploading...' : 'Upload selected photos'}</button></div>{/if}{#if photoUploadError}<p class="error-text">{photoUploadError}</p>{/if}</div>{/if}</section></div>
